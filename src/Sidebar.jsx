  ...
  return (
    <aside className={`bg-slate-900 text-white w-64 h-screen shrink-0 flex flex-col fixed md:static z-20 transition-all duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-64'} md:translate-x-0`}>
      ...
      <nav className="flex-1 px-4 space-y-2 mt-4">
        {navItems.map((item) => (
          <NavLink
            key={item.name}
            to={item.path}
            className={({ isActive }) => `flex items-center gap-3 p-3 rounded-lg transition-all ${isActive ? 'bg-slate-800 border-l-4 border-orange-500' : 'hover:bg-slate-800'}`}
          >
            <item.icon size={20} />
            <span>{item.name}</span>
          </NavLink>
      ...
      <div className="p-4 border-t border-slate-800">
        <NavLink to="/settings" className="flex items-center gap-3 p-3 hover:bg-slate-800 rounded-lg">
          <Settings size={20} />
          <span>Settings</span>
        </NavLink>
      </div>
